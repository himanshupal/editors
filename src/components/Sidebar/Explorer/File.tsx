...
const File = memo(({ content: f, renderInput, level, deleteFileOrFolder, forceRenderInput, isRoot }: IFileProps) => {
	...
	const fileLanguage = getLanguageForFileName(f.name)
		...
		<Fragment>
			...
			{!isRoot && (
				<div
					...
					onClick={(e) => (
						e.stopPropagation(), setSelectedItem(f), isFile(f) ? createModel(fileLanguage, f.name, f.id) : expandFolders([f.id], !f.isExpanded)
					)}
				>
